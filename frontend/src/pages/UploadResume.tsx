...
export default function UploadResume() {
  ...
  const handleAnalyze = async (withJobMatch = false) => {
    ...
    try {
      ...
      const token = auth_session ? JSON.parse(auth_session).token : null;

      const API_URL = import.meta.env.VITE_API_URL || "http://localhost:5001";
      const endpoint = withJobMatch ? "/api/analyze/job-match" : "/api/analyze/resume";
      const body = withJobMatch
      ...
      const data = result.analysis || result.match;
